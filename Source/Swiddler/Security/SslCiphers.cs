...
{
    public static class SslCiphers
    {
        private static readonly Dictionary<int, string> Ciphers = new Dictionary<int, string>
        {
        ...
        {
            if (!Ciphers.TryGetValue(cipherSuite, out string cipherStr))
                cipherStr = $"0x{cipherSuite:X6}";

            return cipherStr;
        }

        public static int[] GetDefaultCiphers(SslProtocols sslProtocol)
        {
            ...
            using (var sslStream = new SslStream(stream, true))
            {
                try
                {
                    sslStream.AuthenticateAsClient("0.0.0.0", null, sslProtocol, false);
                }
                catch { }

                stream.Position = 0;
                return stream.ReadSslClientHello()?.Ciphers ?? new int[0];
            }
        }
    }
}
